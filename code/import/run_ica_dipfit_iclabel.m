...
%   2. Removes M1 and M2 permanently.
...
%      only. HEO, VEO, and Trigger remain unchanged.
...
%
%   'DIPFITCoordTransform'         Default: []
%       Optional manually validated DIPFIT coordinate transformation.
%       When empty, the standard BEM/template lookup is used.
%
...
addParameter(parser, 'RemoveLabels', {'M1', 'M2'}, labelListValidator);
...
addParameter(parser, 'DIPFITCoordTransform', [], ...
    @(x) isempty(x) || isnumeric(x) || ischar(x) || ...
    (isstring(x) && isscalar(x)));
...
if isstring(options.DIPFITCoordTransform)
    options.DIPFITCoordTransform = char(options.DIPFITCoordTransform);
end
...
%% Replace scalp locations with MNI coordinates for the standard BEM model
...
if options.RunDIPFIT
    ...
    [EEG, bemChannelFile] = ...
        lookupMNIForStandardBEM(EEG, scalpIndices);
...
if options.RunDIPFIT
    ...
    dipfitArguments = { ...
        'model', options.DIPFITModel, ...
        'chansel', scalpIndices, ...
        'plotalignment', onOff(options.PlotDIPFITAlignment)};

    if ~isempty(options.DIPFITCoordTransform)
        dipfitArguments = [dipfitArguments, { ...
...
EEG.etc.ica_dipfit_iclabel.dipfitModel = ...
    stringOrEmpty(options.DIPFITModel, options.RunDIPFIT);
EEG.etc.ica_dipfit_iclabel.bemChannelFile = bemChannelFile;
...
%% Save final dataset and QC reports
...
%% ========================================================================
function [EEG, bemChannelFile] = ...
    lookupMNIForStandardBEM(EEG, scalpIndices)
% Look up MNI coordinates associated with the standard DIPFIT BEM model.

dipfitdefs;

if ~exist('template_models', 'var') || isempty(template_models)
    error( ...
        'run_ica_dipfit_iclabel:MissingDIPFITTemplates', ...
        'DIPFIT template model definitions were not found.');
end

modelNames = lower(string({template_models.name}));
bemIndex = find(contains(modelNames, "bem"), 1, 'first');

if isempty(bemIndex)
    if numel(template_models) >= 2
        bemIndex = 2;
    else
        error( ...
            'run_ica_dipfit_iclabel:StandardBEMNotFound', ...
            'The standard BEM model was not found in dipfitdefs.');
    end
end

bemChannelFile = string(template_models(bemIndex).chanfile);
...
% Restore channel types by label in case pop_chanedit modified them.
newLabels = string({EEG.chanlocs.labels});

for channelIndex = 1:numel(newLabels)

    originalIndex = find( ...
        strcmpi(newLabels(channelIndex), cachedLabels), ...
        1, ...
        'first');

    if ~isempty(originalIndex) && isfield(EEG.chanlocs, 'type')
        EEG.chanlocs(channelIndex).type = ...
            char(cachedTypes(originalIndex));
    end
...
if isfield(EEG, 'etc') && ...
    ...
    mostLikelyClass = string( ...
        EEG.etc.ic_classification.ICLabel.classes(classIndices));
