...
%           {'boundary', '100008', '249'}
%
%       Boundary events are retained in the final datasets, whereas
%       markers 100008 and 249 are removed.
%
%   'OutputDir'
...
addParameter(parser, 'IgnoreTypes', {'boundary', '100008', '249'});
...
for recording = 1:3
    ...
    %% Retain boundary events but remove excluded non-experimental markers

    % Experimental markers will be reconstructed from the common sequence.
    % Boundary events are retained because they describe discontinuities.
    % Markers 100008 and 249 are excluded and removed from the final dataset.

    eventTemplate = EEG.event(1);

    keepEvent = false(1, numel(EEG.event));

    for eventIndex = 1:numel(EEG.event)

        currentType = eventTypeToText( ...
            EEG.event(eventIndex).type);

        % Preserve only boundary events.
        keepEvent(eventIndex) = ...
            strcmpi(strtrim(currentType), 'boundary');
