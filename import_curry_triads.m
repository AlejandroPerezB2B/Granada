...
%       └── ...
%
%   This function performs only the initial raw-data import. It does not:
%
%       - filter the EEG;
%       - resample the EEG;
%       - re-reference the EEG;
%       - remove channels;
%       - run artifact correction;
%       - run ICA;
%       - trim the recordings;
%       - synchronise the three members of the triad.
%
% INPUTS
%
%   rawDataDir
...
if exist('eeglab', 'file') ~= 2
    ...
    error( ...
        'import_curry_triads:EEGLABNotFound', ...
...
if exist('pop_fileio', 'file') ~= 2
    ...
    error( ...
        'import_curry_triads:FileIONotFound', ...
...
if exist('pop_saveset', 'file') ~= 2

    error( ...
        'import_curry_triads:PopSaveSetNotFound', ...
        'The EEGLAB function POP_SAVESET was not found.');
...
if exist('eeg_checkset', 'file') ~= 2
    ...
    error( ...
        'import_curry_triads:EegCheckSetNotFound', ...
        'The EEGLAB function EEG_CHECKSET was not found.');
...
uniqueTriadCodes = unique( ...
    {participantRecords.triadCode}, ...
    'stable');
...
for participantIndex = 1:numel(participantRecords)
    ...
    currentLogRow.triad_code = ...
        currentRecord.triadCode;

    currentLogRow.member = ...
        currentRecord.memberIndex;

    currentLogRow.participant_id = ...
        currentRecord.participantId;
    ...
    try
        ...
        if isfile(outputSetFile) && ~options.Overwrite
            ...
            %% Validate the imported EEGLAB structure
            ...
            if isempty(EEG) || ...
                ...
                error( ...
                    'import_curry_triads:InvalidEEGOutput', ...
                    ['POP_FILEIO did not return a valid EEGLAB EEG ' ...
                     'structure for participant %s.'], ...
                    currentRecord.participantId);
            ...
            EEG.etc.triadic_pipeline.import_function = ...
                mfilename;

            EEG.etc.triadic_pipeline.import_date = ...
                datestr(now, 30);

            EEG.etc.triadic_pipeline.triad_code = ...
                currentRecord.triadCode;
            ...
            EEG.etc.triadic_pipeline.raw_files = ...
                curryFiles;

            EEG.etc.triadic_pipeline.preprocessing_applied = ...
                false;

            EEG.etc.triadic_pipeline.synchronisation_applied = ...
                false;
            ...
            currentMember.nSamples = ...
                EEG.pnts * EEG.trials;

            currentMember.durationSeconds = ...
                currentMember.nSamples / EEG.srate;
            ...
            currentLogRow.n_channels = ...
                currentMember.nChannels;

            currentLogRow.sampling_rate_hz = ...
                currentMember.samplingRateHz;

            currentLogRow.n_samples = ...
                currentMember.nSamples;

            currentLogRow.duration_seconds = ...
                currentMember.durationSeconds;

            currentLogRow.n_events = ...
                currentMember.nEvents;

            currentLogRow.event_types = ...
                strjoin(eventTypes, ' | ');
    ...
    triads(triadIndex).members(end + 1, 1) = ...
        currentMember;
